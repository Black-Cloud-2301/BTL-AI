import { targetArray } from '@pages/index';
import { arrLength, isEquals, isExist, swapArray2d } from './functions';

// interface ArrayProps {
//   array: number[][];
//   father: number[][][];
// }

export function DepthFirstSearch(array: number[][]) {
  let t0 = performance.now();
  const search = 0;
  const arrayStatePass = [];
  arrayStatePass.push(array);
  const stack: number[][][] = [];
  stack.push(array);

  while (true) {
    const openQueue: number[][][] = [];
    if (stack.length % 1000 === 0) console.log(stack.length);
    const tempStack = stack.shift();
    if (tempStack) {
      const row = tempStack.findIndex((row) => row.includes(search));
      const col = tempStack[row].indexOf(search);
      if (row === 0) {
        let newTempArray1: number[][] = [];
        for (let i = 0; i < arrLength; i++) {
          newTempArray1[i] = tempStack[i].slice();
        }
        swapArray2d(newTempArray1, row, col, row + 1, col);
        if (!isExist(newTempArray1, arrayStatePass)) {
          openQueue.push([
            [...newTempArray1[0]],
            [...newTempArray1[1]],
            [...newTempArray1[2]],
            [...newTempArray1[3]],
          ]);
          arrayStatePass.push(newTempArray1);
        }

        if (col === 0) {
          let newTempArray2: number[][] = [];
          for (let i = 0; i < arrLength; i++) {
            newTempArray2[i] = tempStack[i].slice();
          }
          swapArray2d(newTempArray2, row, col, row, col + 1);
          if (!isExist(newTempArray2, arrayStatePass)) {
            openQueue.push([
              [...newTempArray2[0]],
              [...newTempArray2[1]],
              [...newTempArray2[2]],
              [...newTempArray2[3]],
            ]);
            arrayStatePass.push(newTempArray2);
          }
        } else {
          if (col === arrLength - 1) {
            let newTempArray3: number[][] = [];
            for (let i = 0; i < arrLength; i++) {
              newTempArray3[i] = tempStack[i].slice();
            }
            swapArray2d(newTempArray3, row, col, row, col - 1);
            if (!isExist(newTempArray3, arrayStatePass)) {
              openQueue.push([
                [...newTempArray3[0]],
                [...newTempArray3[1]],
                [...newTempArray3[2]],
                [...newTempArray3[3]],
              ]);
              arrayStatePass.push(newTempArray3);
            }
          } else {
            let newTempArray4: number[][] = [];
            for (let i = 0; i < arrLength; i++) {
              newTempArray4[i] = tempStack[i].slice();
            }
            swapArray2d(newTempArray4, row, col, row, col - 1);
            if (!isExist(newTempArray4, arrayStatePass)) {
              openQueue.push([
                [...newTempArray4[0]],
                [...newTempArray4[1]],
                [...newTempArray4[2]],
                [...newTempArray4[3]],
              ]);
              arrayStatePass.push(newTempArray4);
            }
            let newTempArray5: number[][] = [];
            for (let i = 0; i < arrLength; i++) {
              newTempArray5[i] = tempStack[i].slice();
            }
            swapArray2d(newTempArray5, row, col, row, col + 1);
            if (!isExist(newTempArray5, arrayStatePass)) {
              openQueue.push([
                [...newTempArray5[0]],
                [...newTempArray5[1]],
                [...newTempArray5[2]],
                [...newTempArray5[3]],
              ]);
              arrayStatePass.push(newTempArray5);
            }
          }
        }
      } else if (row === arrLength - 1) {
        let newTempArray6: number[][] = [];
        for (let i = 0; i < arrLength; i++) {
          newTempArray6[i] = tempStack[i].slice();
        }
        swapArray2d(newTempArray6, row, col, row - 1, col);
        if (!isExist(newTempArray6, arrayStatePass)) {
          openQueue.push([
            [...newTempArray6[0]],
            [...newTempArray6[1]],
            [...newTempArray6[2]],
            [...newTempArray6[3]],
          ]);
          arrayStatePass.push(newTempArray6);
        }
        if (col === 0) {
          let newTempArray7: number[][] = [];
          for (let i = 0; i < arrLength; i++) {
            newTempArray7[i] = tempStack[i].slice();
          }
          swapArray2d(newTempArray7, row, col, row, col + 1);
          if (!isExist(newTempArray7, arrayStatePass)) {
            openQueue.push([
              [...newTempArray7[0]],
              [...newTempArray7[1]],
              [...newTempArray7[2]],
              [...newTempArray7[3]],
            ]);
            arrayStatePass.push(newTempArray7);
          }
        } else {
          if (col === arrLength - 1) {
            let newTempArray8: number[][] = [];

            for (let i = 0; i < arrLength; i++) {
              newTempArray8[i] = tempStack[i].slice();
            }
            swapArray2d(newTempArray8, row, col, row, col - 1);
            if (!isExist(newTempArray8, arrayStatePass)) {
              openQueue.push([
                [...newTempArray8[0]],
                [...newTempArray8[1]],
                [...newTempArray8[2]],
                [...newTempArray8[3]],
              ]);
              arrayStatePass.push(newTempArray8);
            }
          } else {
            let newTempArray9: number[][] = [];
            for (let i = 0; i < arrLength; i++) {
              newTempArray9[i] = tempStack[i].slice();
            }
            swapArray2d(newTempArray9, row, col, row, col - 1);
            if (!isExist(newTempArray9, arrayStatePass)) {
              openQueue.push([
                [...newTempArray9[0]],
                [...newTempArray9[1]],
                [...newTempArray9[2]],
                [...newTempArray9[3]],
              ]);
              arrayStatePass.push(newTempArray9);
            }
            let newTempArray10: number[][] = [];

            for (let i = 0; i < arrLength; i++) {
              newTempArray10[i] = tempStack[i].slice();
            }
            swapArray2d(newTempArray10, row, col, row, col + 1);
            if (!isExist(newTempArray10, arrayStatePass)) {
              openQueue.push([
                [...newTempArray10[0]],
                [...newTempArray10[1]],
                [...newTempArray10[2]],
                [...newTempArray10[3]],
              ]);
              arrayStatePass.push(newTempArray10);
            }
          }
        }
      } else {
        let newTempArray11: number[][] = [];
        for (let i = 0; i < arrLength; i++) {
          newTempArray11[i] = tempStack[i].slice();
        }
        swapArray2d(newTempArray11, row, col, row - 1, col);
        if (!isExist(newTempArray11, arrayStatePass)) {
          openQueue.push([
            [...newTempArray11[0]],
            [...newTempArray11[1]],
            [...newTempArray11[2]],
            [...newTempArray11[3]],
          ]);
          arrayStatePass.push(newTempArray11);
        }
        let newTempArray12: number[][] = [];
        for (let i = 0; i < arrLength; i++) {
          newTempArray12[i] = tempStack[i].slice();
        }
        swapArray2d(newTempArray12, row, col, row + 1, col);
        if (!isExist(newTempArray12, arrayStatePass)) {
          openQueue.push([
            [...newTempArray12[0]],
            [...newTempArray12[1]],
            [...newTempArray12[2]],
            [...newTempArray12[3]],
          ]);
          arrayStatePass.push(newTempArray12);
        }
        if (col === 0) {
          let newTempArray13: number[][] = [];

          for (let i = 0; i < arrLength; i++) {
            newTempArray13[i] = tempStack[i].slice();
          }
          swapArray2d(newTempArray13, row, col, row, col + 1);
          if (!isExist(newTempArray13, arrayStatePass)) {
            openQueue.push([
              [...newTempArray13[0]],
              [...newTempArray13[1]],
              [...newTempArray13[2]],
              [...newTempArray13[3]],
            ]);
            arrayStatePass.push(newTempArray13);
          }
        } else {
          if (col === arrLength - 1) {
            let newTempArray14: number[][] = [];

            for (let i = 0; i < arrLength; i++) {
              newTempArray14[i] = tempStack[i].slice();
            }
            swapArray2d(newTempArray14, row, col, row, col - 1);
            if (!isExist(newTempArray14, arrayStatePass)) {
              openQueue.push([
                [...newTempArray14[0]],
                [...newTempArray14[1]],
                [...newTempArray14[2]],
                [...newTempArray14[3]],
              ]);
              arrayStatePass.push(newTempArray14);
            }
          } else {
            let newTempArray15: number[][] = [];

            for (let i = 0; i < arrLength; i++) {
              newTempArray15[i] = tempStack[i].slice();
            }
            swapArray2d(newTempArray15, row, col, row, col - 1);

            if (!isExist(newTempArray15, arrayStatePass)) {
              openQueue.push([
                [...newTempArray15[0]],
                [...newTempArray15[1]],
                [...newTempArray15[2]],
                [...newTempArray15[3]],
              ]);
              arrayStatePass.push(newTempArray15);
            }
            let newTempArray16: number[][] = [];

            for (let i = 0; i < arrLength; i++) {
              newTempArray16[i] = tempStack[i].slice();
            }
            swapArray2d(newTempArray16, row, col, row, col + 1);
            if (!isExist(newTempArray16, arrayStatePass)) {
              openQueue.push([
                [...newTempArray16[0]],
                [...newTempArray16[1]],
                [...newTempArray16[2]],
                [...newTempArray16[3]],
              ]);
              arrayStatePass.push(newTempArray16);
            }
          }
        }
      }
    }
    const tempOpenQueue = openQueue.slice(0, 4 * 4).sort(function () {
      return Math.random() - 0.5;
    });
    for (let i = 0; i < tempOpenQueue.length; i++) {
      if (isEquals(tempOpenQueue[i], targetArray)) {
        console.log(arrayStatePass);
        console.log('Founded');
        let t1 = performance.now();
        console.log(t1 - t0);
        return arrayStatePass;
      } else {
        stack.unshift(tempOpenQueue[i]);
      }
    }
  }
  return [array];
}
